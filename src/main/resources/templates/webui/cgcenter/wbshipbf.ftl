		...
		<div class="layui-card">
			<div class="layui-card-body" style="padding: 15px;">
				...
				<form class="layui-form" action="" lay-filter="component-form-group">
					...
					<div class="layui-form-item">
						<label class="layui-form-label">视频地址<span class="need">*</span></label>
						<div class="layui-input-inline cz">
							<input type="text" name="orderurl" value="" maxlength="100"
								lay-verify="required" placeholder="" autocomplete="off"
								class="layui-input">
						</div>
						<div class="layui-form-mid layui-word-aux">格式：http://weibo.com/u/1742727537</div>
																																		<div class="layui-form-item" id="postdiv" hidden>
					<label class="layui-form-label"></label>
						<div class="layui-input-inline cz">
						视频名称：<span id="namecontent" style="color:red"></span>     当前视频初始值:<span id="spnum" style="color:red"></span>
						</div>
					</div>
					</div>
					...
					<div class="layui-form-item">
						...
						<div class="layui-input-inline cz">
							<input type="text" name="allnum" value="10000"
 ...
 // 设置最小可选的日期
    function minDate(){
     ...
     $("input[name='orderurl']").blur(function(){
         ...
         layui.$.ajax({
             type: "post",
             url: '/order/getshipinInfor?wburl='+wburl,
             dataType: "json",
             success: function(res){              
       if(res.sucess){
            var data = res.data;
            var play_num = data.play_num;
            var play_name = data.play_name;
            $("#postdiv").show();
            $("#namecontent").text(play_name);
            $("#spnum").text(play_num);
            righturl = 1;
     }
      else{
    ...
    $("input[name='allnum']").on('input propertychange',function(){
        ...
        var goodname = $("#goodname").val();
        	if(sum<10000){
                //$("input[name='allnum']").val(10000); 
                layer.msg("最小数量为10000", {
                    offset: '15px'
                    ,icon: 2
                 });
                sum = 10000;
