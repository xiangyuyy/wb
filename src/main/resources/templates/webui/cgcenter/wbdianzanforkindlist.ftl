			...
			<div class="layui-form layui-card-header layuiadmin-card-header-auto">
				<div class="layui-form-item">
					...
					<div class="layui-inline">
						...
						<div class="layui-input-inline">
							<input type="text" name="orderurl" placeholder="微博地址"
								autocomplete="off" class="layui-input">
						</div>
					</div>

					<!--                                         <div class="layui-inline">
            <label class="layui-form-label">赞类型</label>
            <div class="layui-input-inline">
               <select name="orderkind" lay-verify="required">
        <option value=""></option>
        <option value="1">微博赞</option>
        <option value="2">评论赞</option>
        <option value="3">表情赞</option>
      </select>
            </div>
          </div> -->
					<div class="layui-inline">
...
{{# }}}
				...
				<script type="text/html" id="orderkind">
          {{# if(d.orderkind==1) { }}
            微博赞
{{# } else if(d.orderkind==2) { }}
            评论赞
{{# } else{ }}
             表情赞
{{# }}}
    ...
    return date.Format(d.createtime); 
				...
				<script type="text/html" id="table-content-list">
                    {{# if(d.status==1 || d.status==2 || d.status==7) { }}
...
<!--  <a class="layui-btn layui-btn-normal layui-btn-xs" lay-event="return">退单</a> -->
  ...
  }).use(['index', 'wbdianzanlist', 'table'], function(){
...
/*     layui.$.ajax({
        type: "post",
        url: "/order/synRefundOrder?id=12",
        dataType: "json",
...
error: function (XMLHttpRequest, textStatus, errorThrown) {
    ...
    layer.open({
        type: 2
        ,title: '添加'
        ,content: "/cgcenter/wbdianzanforkind?id="+${goodid!}
        ,maxmin: false
        ,area: ['90%', '90%']
      }); 
    }
    var $ = layui.$, active = {
      ...
      add: function(){
        ...
        layer.open({
          type: 2
          ,title: '添加'
          ,content: '/cgcenter/wbdianzanforkind?id='+id
          ,maxmin: false
          ,area: ['90%', '90%']
        }); 
      },
      adds: function(){
          ...
          layer.open({
              type: 2
              ,title: '批量添加'
              ,content: '/cgcenter/wbdianzanpl?id='+id
              ,maxmin: false
              ,area: ['90%', '90%']
            }); 
        }
